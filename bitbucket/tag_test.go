...
func TestValidateTagNotExisting(t *testing.T) {
	...
	repo := BitbucketProperties{Username: "username", RepoProperties: tagging.RepoProperties{"password", "repo", "tag", "hash", ""}}
...
func TestValidateTagExistingMismatchHash(t *testing.T) {
	...
	gock.New("https://api.bitbucket.org").
		...
		Reply(http.StatusOK).
		JSON(tag)
	repo := BitbucketProperties{Username: "username", RepoProperties: tagging.RepoProperties{"password", "repo", "tag", "not_hash", ""}}
	results := repo.ValidateTag()
	assertTest.False(results.TagDoesntExist)
	assertTest.False(results.TagExistsWithProvidedHash)
}
...
func TestCreateTagSuccessfulWithHostOverride(t *testing.T) {
	...
	gock.New("https://api.personal-bitbucket.com").
		...
		JSON(tag)
	assertTest := assert.New(t)
	repo := BitbucketProperties{Username: "username", RepoProperties: tagging.RepoProperties{"password", "repo", "tag", "hash", "https://api.personal-bitbucket.com"}}
	assertTest.True(repo.CreateTag())
}
...
func TestCreateTagAlreadyExists(t *testing.T) {
	...
	gock.New("https://api.bitbucket.org").
		...
		JSON(tag)
	assertTest := assert.New(t)
	repo := BitbucketProperties{Username: "username", RepoProperties: tagging.RepoProperties{"password", "repo", "test", "hash", ""}}
	assertTest.True(repo.CreateTag())
}
