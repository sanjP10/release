...
func TestCreateTagUnauthorized(t *testing.T) {
	...
	gock.New("https://gitlab.com/").
		...
		Reply(http.StatusUnauthorized)
	assertTest := assert.New(t)
	repo := Properties{Repo: "org/repo", Host: "", Body: "hello", RepoProperties: tag.RepoProperties{"token", "tag", "hash"}}
	assertTest.False(repo.CreateTag())
}
...
func TestCreateTagSuccessful(t *testing.T) {
	...
	assertTest := assert.New(t)
	repo := Properties{Body: "hello", Repo: "org/repo", Host: "", RepoProperties: tag.RepoProperties{"token", "tag", "hash"}}
...
func TestCreateTagSuccessfulWithHostOverride(t *testing.T) {
	...
	gock.New("http://personal-gitlab.com/").
		...
		JSON(body)
	assertTest := assert.New(t)
	repo := Properties{Body: "hello", Repo: "org/repo", Host: "http://personal-gitlab.com", RepoProperties: tag.RepoProperties{"token", "tag", "hash"}}
	assertTest.True(repo.CreateTag())
}
...
func TestCreateTagAndReleaseAlreadyExists(t *testing.T) {
	...
	gock.New("https://gitlab.com/").
		...
		JSON(response)
	assertTest := assert.New(t)
	repo := Properties{Body: "hello", Repo: "org/repo", Host: "", RepoProperties: tag.RepoProperties{"token", "test", "hash"}}
	assertTest.True(repo.CreateTag())
}
