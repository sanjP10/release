...
func TestValidateTagNotExisting(t *testing.T) {
	...
	repo := Properties{Username: "username", Repo: "org/repo", Host: "", RepoProperties: tag.RepoProperties{"password", "tag", "hash"}}
...
func TestValidateTagExistingMismatchHash(t *testing.T) {
	...
	gock.New("https://api.github.com").
		...
		Reply(http.StatusOK).
		JSON(response)
	repo := Properties{Username: "username", Repo: "org/repo", Host: "", RepoProperties: tag.RepoProperties{"password", "tag", "not_hash"}}
	results := repo.ValidateTag()
	assertTest.False(results.TagDoesntExist)
	assertTest.False(results.TagExistsWithProvidedHash)
}
...
func TestCreateTagUnauthorized(t *testing.T) {
	...
	gock.New("https://api.github.com").
		...
		JSON(body)
	assertTest := assert.New(t)
	repo := Properties{Username: "username", Body: "hello", Repo: "org/repo", Host: "", RepoProperties: tag.RepoProperties{"password", "tag", "hash"}}
	assertTest.False(repo.CreateTag())
}
...
func TestCreateTagSuccessfulWithHostOverride(t *testing.T) {
	...
	gock.New("https://api.personal-github.com").
		...
		JSON(body)
	assertTest := assert.New(t)
	repo := Properties{Username: "username", Body: "hello", Repo: "org/repo", Host: "https://api.personal-github.com", RepoProperties: tag.RepoProperties{"password", "tag", "hash"}}
	assertTest.True(repo.CreateTag())
}
...
func TestCreateTagAlreadyExists(t *testing.T) {
	...
	gock.New("https://api.github.com").
		...
		JSON(response)
	assertTest := assert.New(t)
	repo := Properties{Username: "username", Body: "hello", Repo: "org/repo", Host: "", RepoProperties: tag.RepoProperties{"password", "test", "hash"}}
	assertTest.True(repo.CreateTag())
}
