...
func TestValidateTagNotExisting_Github(t *testing.T) {
	...
	repo := Properties{Username: "username", RepoProperties: interfaces.RepoProperties{"password", "repo", "tag", "hash", ""}}
...
func TestValidateTagExistingMismatchHash_Github(t *testing.T) {
	...
	gock.New("https://api.github.com").
		...
		Reply(http.StatusOK).
		JSON(tag)
	repo := Properties{Username: "username", RepoProperties: interfaces.RepoProperties{"password", "repo", "tag", "not_hash", ""}}
	results := repo.ValidateTag()
	assertTest.False(results.TagDoesntExist)
	assertTest.False(results.TagExistsWithProvidedHash)
}
...
func TestCreateTagUnauthorized_Github(t *testing.T) {
	...
	gock.New("https://api.github.com").
		...
		JSON(body)
	assertTest := assert.New(t)
	repo := Properties{Username: "username", Body: "hello", RepoProperties: interfaces.RepoProperties{"password", "repo", "tag", "hash", ""}}
	assertTest.False(repo.CreateTag())
}
...
func TestCreateTagSuccessfulWithHostOverride_Github(t *testing.T) {
	...
	gock.New("https://api.personal-github.com").
		...
		JSON(body)
	assertTest := assert.New(t)
	repo := Properties{Username: "username", Body: "hello", RepoProperties: interfaces.RepoProperties{"password", "repo", "tag", "hash", "https://api.personal-github.com"}}
	assertTest.True(repo.CreateTag())
}
...
func TestCreateTagAlreadyExists_Github(t *testing.T) {
	...
	gock.New("https://api.github.com").
		...
		JSON(tag)
	assertTest := assert.New(t)
	repo := Properties{Username: "username", Body: "hello", RepoProperties: interfaces.RepoProperties{"password", "repo", "test", "hash", ""}}
	assertTest.True(repo.CreateTag())
}
